    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {mcq.options.map((option, index) => {
          ...
          // Dynamic labels: A, B, C, D, E, F...
          const label = String.fromCharCode(65 + index);

          let optionStyle =
            "border-gray-100 bg-gray-50/50 hover:bg-gray-100/60 hover:border-gray-200 text-gray-800 active:scale-[0.995]";
          ...
          if (isAnswered && isCorrectOption) {
            optionStyle =
              "border-emerald-500 bg-emerald-50/80 text-emerald-950 font-medium shadow-sm shadow-emerald-100/30 ring-1 ring-emerald-400/20";
          ...
          return (
            <button
              ...
              id={`mcq-${mcq.id}-option-${label.toLowerCase()}`}
            ...
            >
              <span
                className={`flex items-center justify-center w-6 h-6 rounded-md border text-xs font-bold leading-none shrink-0 font-mono transition-colors duration-200 ${labelStyle}`}
              >
                {label}
              </span>
